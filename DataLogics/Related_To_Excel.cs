...
{
    public class Related_To_Excel
    {
        private readonly CommonDl _commonDl;
        private readonly IConfiguration _config;
        public Related_To_Excel(CommonDl connonDl, [FromServices] IConfiguration config)
        {
            _commonDl = connonDl;
            _config = config;
        }
        ...
        public Common Upload_excel(External_File_request request)
        {
            ...
            using (SqlConnection conn = _commonDl.Connect())
            {
                try
                {
                    ...
                    cmd.Parameters.Add("@RETURN_TEXT", SqlDbType.VarChar, 500);
